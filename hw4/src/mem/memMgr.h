...
#define toSizeT(t)      (t % 8 ? t+8-t%8 : t)  // TODO
//
...
public:
   ...
   void reset(size_t b = 0) {
		...
		#endif // MEM_DEBUG
		// TODO
		MemBlock<T> *prev, *tmpPtr = _activeBlock;
		while(tmpPtr->getNextBlock()){
			prev = tmpPtr;
		...
		if(b == _blockSize || !b){
			//memset(_activeBlock->_begin, 0, _blockSize); // maybe just nothing
		}
		else{
			delete _activeBlock;
   ...
   void  free(T* p) {
	...
	// Called by delete[]
	void  freeArr(T* p) { // p is the begin position (store size_t)
		#ifdef MEM_DEBUG
...
private:
   ...
   MemRecycleList<T>          _recycleList[R_SIZE];
	...
	MemRecycleList<T>* getMemRecycleList(size_t n) {
		size_t m = n % R_SIZE;
		// TODO
		MemRecycleList<T>* tmpPtr = &_recycleList[m];
		for(MemRecycleList<T>* i=tmpPtr; i; i = i->_nextList){
			if(n == i->_arrSize){
				return i;
			}
			tmpPtr = i;
		}
		tmpPtr->_nextList = new MemRecycleList<T>(n);
		return tmpPtr->_nextList;
	}
	// t is the #Bytes requested from new or new[]
	// Note: Make sure the returned memory is a multiple of SIZE_T
	T* getMem(size_t t) {
		T* ret = 0; // T is TestObj
		// return the pointer to memory position
		#ifdef MEM_DEBUG
		...
		// Obj[n], n means how many
		int n = getArraySize(t);
		MemRecycleList<T>* _rlist = getMemRecycleList(n);
		if(_rlist->_first){
			ret = _rlist->popFront();
			#ifdef MEM_DEBUG
			cout << "Recycled from _recycleList[" << _rlist->_arrSize << "]..." << _rlist << endl;
			#endif // MEM_DEBUG
			return ret;
		}
		...
		// memory not enough
			...
			if(remainsize > S){
				int rn = getArraySize(remainsize);
				_rlist = getMemRecycleList(rn);
				size_t arrMemSize = SIZE_T + S * rn;
				_activeBlock->getMem(arrMemSize, ret);
				_rlist->pushFront(ret);
				#ifdef MEM_DEBUG
				cout << "Recycling " << ret << " to _recycleList[" << rn << "]\n";
				#endif // MEM_DEBUG
			}
