...
getChar(istream& istr)
{
   ...
   switch (ch) {
      ...
      case ESC_KEY:{
      	char combo1 = mygetc(istr);
	if(combo1 == '[') { // '[' for ascii:91, both arrow & mod
		char combo2 = mygetc(istr);
		if(combo2 >= 65 && combo2 <= 68) {
			return returnCh(ARROW_KEY_FLAG + combo2);
		} else if (combo2 == 70) {
			return END_KEY;
		} else if (combo2 == 72) {
			return HOME_KEY;
		} else if (combo2 >= 49 && combo2 <= 54) {
			char combo3 = mygetc(istr);
			if(combo3 == MOD_KEY_DUMMY) {
				return returnCh(MOD_KEY_FLAG + combo2);
			}
		}
	}
